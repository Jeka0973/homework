...
class StoreThunk extends Store {
  ...
  dispatch(action) {
    ...
    super.dispatch(action) //тут наследуем то, что в Store; store.state не store.state() потому что геттер!!)
  }
}
...
const store = new StoreThunk(combineReducers(reducers))

store.dispatch(actionPromise('getCategories', gqlFunc(queryRootCategories, rootCategories)))
store.subscribe(() => getCategories(store.state))
...
window.onhashchange = async function () {
  ...
  if (section == 'category') {
    ...
    store.subscribe(() => getGoods(store.state))
  }

  if (section == 'good') {
    store.dispatch(actionPromise('getOneGood', await gqlFunc(queryGood, {q: `[{"_id":"${id}"}]`})))
    store.subscribe(() => getOneGood(store.state))
  }
  ...
  if (section == 'orderHistory') {
    ...
    if (login) {
      ...
      store.subscribe(() => getOrderHistory(store.state))
    } else {
